...
```mermaid
graph TB
    subgraph "BYVALVER - NULL-BYTE ELIMINATION FRAMEWORK"
        Input[Input Shellcode]
        Output[Output Shellcode]
        ...
        subgraph "Processing Modes"
            Biphasic[Biphasic Processing]
            PIC[PIC Generation]
            XOR[XOR Encoding]
        end

        subgraph "Strategy Pattern"
            SPRegistry[Strategy Registry]
            SPInterface[Strategy Interface]
            SPSpecific[Specific Strategies]
        end
    ...
    SPRegistry --> SPInterface
    SPInterface --> SPSpecific
    SPSpecific --> Strategies
```
...
classDiagram
    class StrategyInterface {
        <<interface>>
        +name: string
        +can_handle(insn): boolean
        +get_size(insn): size_t
        +generate(buffer, insn): void
        +priority: int
    }
    ...
    class StrategyRegistry {
        ...
        +init_strategies(use_ml)
    }

    class MOVStrategy {
        +can_handle(insn)
        +get_size(insn)
        +generate(buffer, insn)
    }
    ...
    class CoreEngine {
        -remove_null_bytes(shellcode, size)
        -biphasic_process(shellcode, size)
        -apply_obfuscation(shellcode, size)
    }

    StrategyInterface <|-- MOVStrategy
    StrategyInterface <|-- ArithmeticStrategy
    StrategyInterface <|-- MemoryStrategy
    StrategyInterface <|-- JumpStrategy

    StrategyRegistry o-- StrategyInterface
    CoreEngine --> StrategyRegistry
```

### 3. ML Strategist Architecture

```mermaid
graph TB
    subgraph "ML Strategist"
        MLInit[Initialize ML Model]
        FeatureExtraction[Extract Instruction Features]
        NeuralNetwork[Neural Network Inference]
        StrategyPrioritization[Reprioritize Strategies]
        Feedback[Provide Feedback]
        ModelUpdate[Update Model Weights]
    end

    subgraph "Instruction Processing"
        Insn[Input Instruction]
        Features[Feature Vector]
        NNOutput[NN Output - Strategy Scores]
        RankedStrategies[Ranked Strategy List]
    ...
    Insn --> FeatureExtraction
    FeatureExtraction --> Features
    Features --> NeuralNetwork
    NeuralNetwork --> NNOutput
    NNOutput --> StrategyPrioritization
    StrategyPrioritization --> RankedStrategies
    RankedStrategies --> AppliedStrategy

    AppliedStrategy --> Feedback
    Feedback --> ModelUpdate
    Feedback --> Metrics

    MLInit --> NeuralNetwork
    Metrics --> Accuracy
    Metrics --> Predictions
```
...
flowchart TD
    ...
    B --> C{Input is Directory?}
    C -->|Yes| D[Batch Processing]
    C -->|No| E[Single File Processing]

    D --> F[Find Matching Files]
    F --> G[Process Each File]
    G --> H[Apply Processing Pipeline]

    E --> H
    H --> I[Load Shellcode from File]
    I --> J[Disassemble with Capstone]
    J --> K{Use Biphasic?}
    K -->|Yes| L[Obfuscation Pass]
    K -->|No| M[Null-Byte Elimination]
    L --> M
    M --> N{Use ML Strategist?}
    N -->|Yes| O[ML-Based Strategy Selection]
    N -->|No| P[Priority-Based Strategy Selection]
    O --> Q[Apply Selected Strategy]
    P --> Q
    Q --> R[Generate New Shellcode]
    R --> S{XOR Encode?}
    S -->|Yes| T[Add Decoder Stub + Encode]
    S -->|No| U[Direct Output]
    T --> V[Verify No Null Bytes]
    U --> V
    V --> W[Write to Output File]
    W --> X[End]
...
graph TB
    subgraph "Batch Processing System"
        BPInput[Input Directory]
        BPOutput[Output Directory]
        BPConfig[Batch Configuration]

        subgraph "Directory Scanner"
            FindFiles[Find Files]
            FilePattern[Pattern Matching]
            Recursive[Recursive Scan]
        end

        subgraph "File Processor"
            FP[File Processor]
            ProcessingPipe[Processing Pipeline]
        end
    ...
    BPConfig --> FP
    FP --> ProcessingPipe
    ProcessingPipe --> BPOutput
    FP --> Stats
    Stats --> Track
```

### 6. Biphasic Processing Architecture

```mermaid
graph LR
    subgraph "BIPHASIC PROCESSING"
        subgraph "PASS 1: Obfuscation"
            ODisasm[Disassemble Input]
            OStrategies[Obfuscation Strategies]
            OProcess[Apply Obfuscation]
            OOutput[Obfuscated Output]
        end

        subgraph "PASS 2: Null-Byte Elimination"
            NDisasm[Disassemble Obfuscated]
            NStrategies[Null-Byte Elimination Strategies]
            NProcess[Apply Null Elimination]
            NOutput[Clean Output]
        end
    end

    ODisasm --> OStrategies
    OStrategies --> OProcess
    OProcess --> OOutput
    OOutput --> NDisasm
    NDisasm --> NStrategies
    NStrategies --> NProcess
    NProcess --> NOutput
```
