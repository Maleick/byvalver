...
for file in "${all_files[@]}"; do
    ...
    while IFS= read -r line; do
        ...
        for shellcode in $potential_shellcodes; do
            ...
            extract_and_save "$hex_string" "$file" && continue
        done
    done < <(grep -i "shellcode\|\\\\x\|0x\|\\x" "$file" 2>/dev/null)
    ...
    # Pattern 3: Look at the entire file for hex patterns
    if [ $count -lt 50 ]; then
        # Look for lines that might contain hex dump format and extract hex values
        hex_chunks=$(grep -oE '[0-9a-fA-F]{2} [0-9a-fA-F]{2} [0-9a-fA-F]{2}' "$file" | tr -d ' \n' 2>/dev/null)
        
        if [ -n "$hex_chunks" ] && [[ "$hex_chunks" == *"00"* ]]; then
            extract_and_save "$hex_chunks" "$file" && continue
        fi
    fi

    # Pattern 4: Look for potential C-style shellcode arrays
    c_shellcode=$(grep -oE '0x[0-9a-fA-F]{2},?\s*0x[0-9a-fA-F]{2}' "$file" | tr -d 'x, \n\t' | sed 's/0*//g' 2>/dev/null)
    ...
    if [ -n "$c_shellcode" ] && [[ "$c_shellcode" == *"00"* ]] && [ ${#c_shellcode} -gt 10 ]; then
        ...
        extract_and_save "$c_shellcode" "$file" && continue
    fi
done
